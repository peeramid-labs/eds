...
describe("Installer", function () {
  ...
  it("Only owner can install distributors", async function () {
    ...
    ).to.emit(installer, "DistributorAdded");
  ...
  it("Only owner can remove distributors", async function () {
    ...
    expect(
      await installer.connect(owner).removeDistributor(distributor.address)
  ...
  it("Anyone can instantiate from whitelisted distributors", async function () {
    ...
    expect(
      await installer
        .connect(deployer)
        .install(distributor.address, cloneDistributionId, "0x")
    ).to.emit(installer, "Installed");
  });
  it("can List distributors", async function () {
    await installer.connect(owner).addDistributor(distributor.address);
    expect(await installer.connect(owner).getDistributors()).to.be.deep.eq([
      distributor.address,
    ]);
  });
  it("Can get instances by id", async function () {
    ...
    expect(
      (await installer.connect(owner).getInstance(instanceNum)).length
    ).to.be.eq(1);
    let instanceAddress = (
      await installer.connect(owner).getInstance(instanceNum)
    )[0];
  ...
  it("Allows only valid instances to call target", async () => {
    ...
    const instanceNum = await installer.connect(owner).getInstancesNum();
    let instanceAddress = (
      await installer.connect(target).getInstance(instanceNum)
    )[0];
    await expect(
      installer
        .connect(target)
